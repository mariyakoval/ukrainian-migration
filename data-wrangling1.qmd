...
rf_model <- randomForest(factor(employed_binary) ~ ., data = model_data, ntree = 500, importance = TRUE)
...
print(rf_reduced)
model_data$employed_binary <- as.numeric(as.character(model_data$employed_binary))
model1 <- glm(employed_binary ~ ., data = model_data_reduced, family = binomial)
hoslem.test(model_data_reduced$employed_binary, fitted(model1), g = 10)
