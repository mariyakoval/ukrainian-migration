...
# common activity levels and their Russian labels
activity_levels <- c(
  "Работал(-а)", "Учился(-ась)", "Занимался(-ась) домом", 
  "Вышел(-ла) на пенсию", "Нетрудоустроенный(-ая)", "Другое"
)

# Table for Ukraine (before move)
act_ukr_table <- combined_data %>%
  filter(country == "Estonia") %>%
  mutate(
    demographics_resp_activity = fct_recode(
    ...
    demographics_resp_activity = factor(demographics_resp_activity, levels = activity_levels)
  ) %>%
...
# Table for Estonia (after move)
act_host_table <- combined_data %>%
  filter(!is.na(host_country_work_coa), country == "Estonia") %>%
  mutate(
    host_country_work_coa = fct_recode(
    ...
    host_country_work_coa = factor(host_country_work_coa, levels = activity_levels)
  ) %>%
  count(country, host_country_work_coa) %>%
  mutate(percent = n / sum(n) * 100)

# Plot for Ukraine
act_ukr <- ggplot(act_ukr_table, aes(x = country, y = percent, fill = demographics_resp_activity)) +
...
# Plot for Estonia
act_host <- ggplot(act_host_table, aes(x = country, y = percent, fill = host_country_work_coa)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(), legend.position = "none") +
  guides(fill = guide_legend(title = "Тип деятельности")) +
  scale_fill_brewer(palette = "Dark2", drop = FALSE) +
  ylab(element_blank()) +
  ggtitle(" ", subtitle = "Занятость в Эстонии")

# Shared legend
shared_legend <- get_legend(
  ggplot(act_ukr_table, aes(x = country, y = percent, fill = demographics_resp_activity)) +
    geom_bar(stat = "identity") +
    scale_fill_brewer(palette = "Dark2", drop = FALSE) +
    guides(fill = guide_legend(title = "Тип деятельности")) +
...
**Estonia graphs for popular article in English**
...
combined_df |>
  ...
  geom_text(aes(label = paste0(Percent, "%")), 
            hjust = -0.1, size = 3) +
  coord_flip() +
  xlab("Needs") + ylab("Percent") +
  ggtitle("Socioeconomic needs of Ukrainians") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none"
  ) +
...
act_host <- combined_data |>
  ...
  guides(fill = guide_legend(title = "Type of activity")) +
  scale_fill_brewer(palette = "Dark2") +
  theme(legend.position = "none") +
  ylab(element_blank()) +
  ggtitle( " ",
           subtitle = "Activity in Estonia"
  )
...
act_ukr <- combined_data |>
  ...
  theme(legend.position = "none" )+
  ggtitle(
    "Work activity of Ukrainians before and after moving to Estonia (2023)",
    subtitle = "Activity in Ukraine"
  )
...
combined_data |>
  ...
  mutate(demographics_educ_level = case_when(
    grepl("tech", demographics_educ_level, ignore.case = TRUE) ~ "Technical",
  ...
  scale_fill_brewer(palette = "Dark2") +
  ggtitle("Education level by age groups of Ukrainians in Estonia (2023)")
...
combined_df |>
  ...
  xlab("Reason") + 
  ylab("Percent") +
  ggtitle("The main obstacles to employment for Ukrainians") +
  theme_minimal() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 35,  hjust = 1, size = 6),
        legend.position = "none") +
...
combined_df |>
  ...
  mutate(Component = fct_recode(Component,
                                ...
                                "None" = "none",
                                "Social media comments" = "comments_social_media",
                                "Discrimination" = "discriminatory_behavior",
                                "Other" = "other"
  )) |>
  mutate(Percent = round(Total_Proportion * 100, 2)) |>
  select(Component, Percent, Country) |>
  ...
  geom_text(aes(label = paste0(Percent, "%")), 
            hjust = -0.1, size = 3) +
  xlab("Type of hostility") + 
  ylab("Percent") +
  ggtitle("The main types of discrimination and social exclusion") +
  theme_minimal() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 35,  hjust = 1, size = 6),
        legend.position = "none") +
...
combined_df |>
  ...
  geom_text(aes(label = paste0(Percent, "%")), 
            hjust = -0.1, size = 3) +
  xlab("Incomr source") + ylab("Percent") +
  ggtitle("Sources of income of Ukrainians in Estonia") +
  theme_minimal() +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 35,  hjust = 1, size = 6),
        legend.position = "none") +
...
eng_labels <- c(
  ...
  "fewer" = "Less than before the war",
  "more" = "More than before the war",
...
# Generate and format table
make_prop_table(data_ee$economic_capicity_L13_SS_AFF_GOODS, na.rm = TRUE) |>
  mutate(Category = eng_labels[Category]) |>
  kable(booktabs = TRUE, align = "c",
        col.names = c("Answer", "Count", "Percent")) |>
  kable_styling(latex_options = c("striped"), font_size = 12) |>
  column_spec(1, bold = TRUE) |>
  row_spec(0, bold = TRUE, color = "white", background = "gray") |>
  add_header_above(c("Ability to purchase goods compared to pre-war times" = 3))
