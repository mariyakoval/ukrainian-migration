...
combined_hh <-readRDS("Data/combined_hh")
combined_ind <-readRDS("Data/combined_ind")
```
...
combined_ind |>
  filter(!is.na(host_country_work_coa), !country %in% c("Hungary", "Bulgaria", "construct", NA)) |>
  ...
  mutate(employment_status = case_when(
    ...
    grepl("student|trainee|illness|housekeeping|other", host_country_work_coa, ignore.case = TRUE) ~ "Unemployed",
    TRUE ~ "Other"
  )) |>

  mutate(unemployed_type = case_when(
    grepl("student", host_country_work_coa, ignore.case = TRUE) ~ "Student",
    grepl("trainee", host_country_work_coa, ignore.case = TRUE) ~ "Trainee",
    grepl("illness", host_country_work_coa, ignore.case = TRUE) ~ "Long-term Illness",
    grepl("housekeeping", host_country_work_coa, ignore.case = TRUE) ~ "Housekeeping",
    grepl("other", host_country_work_coa, ignore.case = TRUE) ~ "Other",
    TRUE ~ NA_character_
  )) |>

  mutate(unemployed_type = ifelse(employment_status == "Employed", "Employed",
                           ifelse(employment_status == "Retired", "Retired", unemployed_type))) |>
  
  filter(!is.na(unemployed_type)) |>
  count(country, unemployed_type) |>
  group_by(country) |>
  mutate(percent = n / sum(n) * 100) |>
  
  ggplot(aes(x = country, y = percent, fill = unemployed_type)) +
  geom_col(position = "stack") +
  xlab("Country") +
  ylab("Percent") +
  ggtitle("Employment Activity Type by Country") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
#During the past 7 days, did (this person) work for someone else for pay, for one
# or more hours?
make_prop_table(data_eu_ind$SE2_SS_WORK)
